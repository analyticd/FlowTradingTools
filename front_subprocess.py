"""
Connect to Front through arena_python subprocess
...
class arena_python():
    def __init__(self, username, password):
        self.production_server = 'ldnfrl01:7101'
...
class FrontConnection():
    ...
    def new_trades(self,date='2015-02-23 00:00'):
        ...
        """
        #it's important to change the date slightly at every call
        ##Note 2: above doesn't work. New records get refreshed but old records, if changed, are not updated.
        command_line = 'U:arena_python.exe arena_python_test.py' + ' '+ username + ' ' + password + ' ' + date
        proc = Popen(command_line,bufsize=-1,stdin=PIPE, stdout=PIPE, stderr=STDOUT)
        out_proc = proc.communicate()
        out =  eval(res[0].split('\r\n')[0])
        res = out[1][0]
...
def main(argv):                         
    if argv[1]=='new_trades':
        ...
        #fc.disconnect()
        proc = Popen('U:arena_python.exe arena_python_test.py',bufsize=-1,stdin=PIPE, stdout=PIPE, stderr=STDOUT)
...
def new_trades(username, password, date):
    """
    Function to load new_trades from Front. Function is called by FrontPnL > getNewTrades (now obsolete?).
    Keyword argument:
    date : defaults to '2015-02-23 00:00' if not specified 
    """
    command_line = 'U:arena_python.exe arena_python_test.py new_trades' + ' '+ username + ' ' + password + ' ' + date
    proc = Popen(command_line,bufsize=-1,stdin=PIPE, stdout=PIPE, stderr=STDOUT)
    out_proc = proc.communicate()
    out =  eval(out_proc[0].split('\r\n')[0])
    res = out[1][0]
    if len(res) == 0: # no new trades yet
        output = pandas.DataFrame(columns=['trdnbr','insid','isin','trade_price','quantity','trade_time','portfolio','trade_curr','status','Trader','Counterparty','Salesperson','Sales Credit','Sales Credit MarkUp'])
    else:
